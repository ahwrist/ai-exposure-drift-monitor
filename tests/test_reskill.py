...
class TestEstimateReskillDifficulty:
    ...
    def test_low_exposure_group_higher_difficulty(self, reference_rates) -> None:
    ...
    def test_unknown_soc_returns_default(self, reference_rates) -> None:
        difficulty = estimate_reskill_difficulty("99-0000", reference_rates)
        assert difficulty == 0.5

    def test_difficulty_in_unit_range(self, reference_rates) -> None:
...
class TestScoreReskillUrgency:
    ...
    def test_high_exposure_high_urgency(self) -> None:
        ...
        exposure = ExposureScore(
            role_id="X", theoretical=0.94, observed=0.33,
            blended=0.8, tier=ExposureTier.CRITICAL, soc_major_group="15-0000",
        )
        drift = DriftResult(
            entity_id="X", direction=DriftDirection.ACCELERATING,
            magnitude=2.0, p_value=0.01, trend_slope=0.08, n_periods=6,
        )
    ...
    def test_low_exposure_low_urgency(self) -> None:
        ...
        exposure = ExposureScore(
            role_id="X", theoretical=0.1, observed=0.02,
            blended=0.05, tier=ExposureTier.LOW, soc_major_group="53-0000",
        )
    ...
    def test_no_drift_data_still_scores(self) -> None:
        ...
        exposure = ExposureScore(
            role_id="X", theoretical=0.5, observed=0.2,
    ...
    def test_custom_weights(self) -> None:
        ...
        exposure = ExposureScore(
            role_id="X", theoretical=0.5, observed=0.3,
            blended=0.4, tier=ExposureTier.MODERATE, soc_major_group="15-0000",
        )
        ...
        heavy_exposure = score_reskill_urgency(
            exposure, None, headcount=10, reskill_difficulty=0.5,
    ...
    def test_score_in_unit_range(self) -> None:
        exposure = ExposureScore(
            role_id="X", theoretical=0.99, observed=0.99,
            blended=0.99, tier=ExposureTier.CRITICAL, soc_major_group="15-0000",
        )
        drift = DriftResult(
            entity_id="X", direction=DriftDirection.ACCELERATING,
            magnitude=5.0, p_value=0.001, trend_slope=0.2, n_periods=8,
        )
    ...
    def test_tier_assignment(self) -> None:
        ...
        for blended, expected_tier in [
            ...
            exposure = ExposureScore(
                role_id="X", theoretical=blended, observed=blended,
                blended=blended, tier=ExposureTier.from_score(blended),
                soc_major_group="15-0000",
            )
            # Use weights that make score ≈ blended
            result = score_reskill_urgency(
                exposure, None, headcount=1, reskill_difficulty=blended,
...
class TestOrgUrgency:
    ...
    def test_org_urgency_sorted_descending(self, sample_roles, sample_scores, reference_rates) -> None:
        results = score_org_urgency(sample_roles, sample_scores, None, reference_rates)
        scores = [r.score for r in results]
        assert scores == sorted(scores, reverse=True)

    def test_org_urgency_covers_all_roles(self, sample_roles, sample_scores, reference_rates) -> None:
        results = score_org_urgency(sample_roles, sample_scores, None, reference_rates)
