...
class TestGenderDisparity:
    ...
    def test_gender_segments_created(
    ...
    def test_disparity_ratio_near_one_for_balanced(self) -> None:
        ...
        roles = [
            Role(
                role_id="A", title="Role A", soc_code="15-1252", headcount=10, gender_pct_female=0.5
            ),
            Role(
                role_id="B", title="Role B", soc_code="15-1252", headcount=10, gender_pct_female=0.5
            ),
        ]
        scores = [
            ExposureScore(
...
class TestEducationDisparity:
    ...
    def test_education_segments_created(
        self, sample_roles: list[Role], sample_scores: list[ExposureScore]
    ) -> None:
    ...
    def test_education_headcount_sums(
        self, sample_roles: list[Role], sample_scores: list[ExposureScore]
    ) -> None:
        segments = analyze_education_disparity(sample_roles, sample_scores)
...
class TestPayBandDisparity:
    ...
    def test_pay_band_segments_created(
        self, sample_roles: list[Role], sample_scores: list[ExposureScore]
    ) -> None:
    ...
    def test_pay_band_values_formatted(
        self, sample_roles: list[Role], sample_scores: list[ExposureScore]
    ) -> None:
        segments = analyze_pay_band_disparity(sample_roles, sample_scores)
        for seg in segments:
...
class TestAllDisparities:
    ...
    def test_all_segment_types_present(
        self, sample_roles: list[Role], sample_scores: list[ExposureScore]
    ) -> None:
