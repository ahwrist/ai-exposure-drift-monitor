...
class TestComputeExposureIndex:
    ...
    def test_high_exposure_role(self, sample_roles, reference_rates) -> None:
    ...
    def test_low_exposure_role(self, sample_roles, reference_rates) -> None:
    ...
    def test_missing_soc_returns_zero(self, reference_rates) -> None:
    ...
    def test_custom_weights(self, sample_roles, reference_rates) -> None:
        ...
        score_theo_heavy = compute_exposure_index(
            sample_roles[0], reference_rates,
            weight_theoretical=0.8, weight_observed=0.2,
        )
    ...
    def test_score_normalized_to_unit_range(self, sample_roles, reference_rates) -> None:
        """All scores must be in [0, 1]."""
        for role in sample_roles:
            score = compute_exposure_index(role, reference_rates)
            assert 0.0 <= score.blended <= 1.0

    def test_tier_assignment_critical(self, reference_rates) -> None:
        ...
        score = compute_exposure_index(
            role, reference_rates,
            weight_theoretical=1.0, weight_observed=0.0,
        )
...
class TestOrgExposure:
    ...
    def test_compute_org_exposure(self, sample_roles, reference_rates) -> None:
        scores = compute_org_exposure(sample_roles, reference_rates)
        assert len(scores) == len(sample_roles)

    def test_org_mean_exposure_weighted(self, sample_roles, reference_rates) -> None:
        scores = compute_org_exposure(sample_roles, reference_rates)
        mean = org_mean_exposure(sample_roles, scores)
        assert 0.0 < mean < 1.0

    def test_department_exposure(self, sample_roles, reference_rates) -> None:
        scores = compute_org_exposure(sample_roles, reference_rates)
